...
impl Png {
    // STANDARD_HEADER: 这是 PNG 图像文件的标准头部。PNG 文件头部包含 8 个固定字节，用于标识文件的类型。
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self, Self::Error> {
        ...
        if header != Png::STANDARD_HEADER {
            return Err("Png Header is not valid");
        }
        let mut chunks = Vec::new();
        let mut index = 8;
        while index < bytes.len() {
            let length_bytes = bytes[index..index + 4].try_into().unwrap();
            let length = u32::from_be_bytes(length_bytes) as usize;
            // 计算当前块的结束位置。PNG 块的结构包括：长度（4 字节）+ 类型（4 字节）+ 数据（length 长度）+ CRC（4 字节）。
            let chunk_end = index + 4 + 4 + length + 4;
            if chunk_end > bytes.len() { return Err("PNG chunk is too long"); }
